...
class MultiLayerPERCEPTRONNeuralNetwork:
    def __init__(self, layers: List[int], type_of_activation_function: str = "sigmoid", seed: Optional[int] = None):
        assert len(layers) >= 2, "Network must have at the very least input and output layers."
        self.layers = layers
        if type_of_activation_function not in _TYPES_OF_ACTIVATIONS:
            raise ValueError(f"Unsupported activation function type: '{type_of_activation_function}'. Supported types are the following: {list(_TYPES_OF_ACTIVATIONS.keys())}")
        self.activation_name = type_of_activation_function
        self.activation, self.activation_derivative = _TYPES_OF_ACTIVATIONS[type_of_activation_function]
    ...
    def compute_neuron_input(self, layer_index: int, neuron_index: int, input_vector: numpy.ndarray) -> float:
        if layer_index < 1 or layer_index >= len(self.layers):
            raise IndexError("layer_index must be between 1 and number_of_layers - 1 (inclusive).")
        W = self.weights[layer_index - 1]
        b = self.biases[layer_index - 1]
        weights_to_neuron = W[:, neuron_index]
        total_input = float(numpy.dot(input_vector, weights_to_neuron) + b[neuron_index])
        return total_input
        
    def feed_forward_pass(self, x: numpy.ndarray) -> Tuple[List[numpy.ndarray], List[numpy.ndarray]]:
        if x.ndim != 1:
            raise ValueError("Input x must be a 1-dimensional (1D) numpy array for single forward pass.")
    ...
    def forward_batch(self, X: numpy.ndarray) -> numpy.ndarray:
        if X.ndim != 2:
            raise ValueError("Input X must be a 2-dimensional (2D) numpy array (n_samples, n_features)")
    ...
    def create_summary(self) -> str:
        returnedString = f"MultiLayerPERCEPTRONNeuralNetwork architecture: {self.layers}\n"
        returnedString += f"Type Of Activation Function Used: {self.activation_name.capitalize()}\n"
...
if X_all.shape[1] < expected_input_neurons:
    raise ValueError(f"CSV has {X_all.shape[1]} columns but expected at least {expected_input_neurons} input features (columns).")
X_inputs = X_all[:, :expected_input_neurons]

layers = [4, 3, 2]
mlpnn = MultiLayerPERCEPTRONNeuralNetwork(layers=layers, type_of_activation_function="sigmoid", seed=42)

print("\nMulti-Layer PERCEPTRON Neural Network Summary:")
...
for index, (input, output) in enumerate(zip(X_inputs, outputs), start=1):
    print(f"Sample {index}: input={numpy.round(input, 6)} --> output={numpy.round(output, 6)}")

sample0 = X_inputs[0]
neuron_total = mlpnn.compute_neuron_input(layer_index=1, neuron_index=0, input_vector=sample0)
